class RuneCompile:
		...
		self.symbols = {
			"pf": {
				...
				"rune": {
					...
					"str": { "type": "type_keyword", "usages": 0, "exist": True, "modification": { "args": ["int", "length"] } }
				}
			},
			"id": [],
			"___literals": {},
			"___preserved_keywords": ["int", "str", "fn", "boolean", "def", "pf"],
	...
	def parser(self, all_tokens):
		...
		for token in all_tokens:
			...
			if definition in ( "Identity", "colon" ) or definition[-7:] == "Literal":
	...
	def attention(self, all_tokens):
		info = {"type": "¿none?", "pf":"global", "referVoid": False}# standard prefix is always global
		reparse = []
		trigger = None
		reparse_bool = False
		nest_tracker = []

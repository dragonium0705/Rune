...
def compile(source, filename_output="__0utput.s"):
	symbol_table = { 
		...
		"___Literals": [],
		"__pkeywords": ["int", "str", "float", "double", "large", "boolean"],
		"__type_keywords": ["int", "str", "float", "double", "large", "boolean"]
	}
	
	if rune_debug:
		print(first_pass_tokens)
		print(second_pass_items)
	first_pass_tokens = first_pass(source, symbol_table)
...
def first_pass(source, symbol_table):
	tokens = []
	...
	for symbol in source:
		...
		if symbol == "\n":
			...
			if str_block:
				str_block = False
				new_token = [current_token.replace("\\n", "\n"), "strLiteral", (nl_c, line_c-len(current_token))]
				tokens.append(new_token)
				current_token = ""
			else:
				str_block = True
			...
			if len(current_token.strip()) != 0 and current_token in symbol_table["__type_keywords"]:
				new_token = [current_token, "type_keyword", (nl_c, line_c-len(current_token))]
				current_token = ""
				tokens.append(new_token)
			elif len(current_token.strip()) != 0:
				new_token = back_tokenize(current_token) 
				new_token.append((nl_c, line_c-len(current_token)))
				current_token = ""
				tokens.append(new_token)
		...
		if rune_debug:
			print(f"{current_token} == {nl_c} == {line_c} == << {symbol} >>")
...
def attention(tokens_to_parse, symbol_table):
	...
	for index, token in enumerate(tokens_to_parse):
		if token[1] == "Identity":
			...
			elif hashmap["type"] == "type¿?":
				hashmap["type"] = "var_declare"
				hashmap["item"] = token[0]
			...
			elif hashmap["type"] == "var_init":
				if hashmap["as"] == token[1][:-7]:
					hashmap["type"] = "var_init_¡!"
					hashmap["value"] = token[0]
					
		elif token[1] == "RBracket":
			if hashmap["type"] == "fnCallRequest":
				hashmap["type"] = "fnCall"
				
		elif token[1] == "type_keyword":
			if index == 0 :
				hashmap["type"] = "type¿?"
				hashmap["as"] = token[0]
				
		elif token[1] == "equal":
			if hashmap["type"] == "var_declare":
				hashmap["type"] = "var_init"
...
def third_pass(datas, symbol_table):
	...
	for liter in symbol_table["___Literals"]:
		print(liter)
		if liter[1] == "str":
			give_writings["data"].append( f"	___{liter[0].replace(' ', '_').replace('\n','_')}__{liter[1]}___: .ascii \"{liter[0]}\"\n")
			give_writings["data"].append( f"	___{liter[0].replace(' ', '_').replace('\n','_')}__{liter[1]}__length___: .quad {len(liter[0])}\n")
	...
	for data in datas:
		...
		if data["type"] == "fnCall":
			...
			args = data["args"]
			if args[0][1] == "str":
				my_str+=f"	adr x10, ___{args[0][0][1:-1].replace(' ', '_').replace('\n','_')}__str___\n	adr x0, ___{args[0][0][1:-1].replace(' ', '_').replace('\n','_')}__str__length___\n	ldr x11, [x0]\n"
			
			my_str += f"	bl {from_pf}_{fn_name} \n"
			give_writings["main"].append(my_str)
			
		elif data["type"] == "var_init_¡!":
			if data["as"] == "int":
				from_pf = data["pf"]
				name = data["item"]
				value = data["value"]
				my_str = f"	{from_pf}__{name}: .word {value}\n"
				give_writings["data"].append(my_str)
				
		elif data["type"] == "var_declare":
			if data["as"] == "int":
				from_pf = data["pf"]
				name = data["item"]
				
				my_str = f"	{from_pf}__{name}: .word 0\n"
				give_writings["data"].append(my_str)
